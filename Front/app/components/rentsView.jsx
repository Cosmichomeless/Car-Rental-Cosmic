...
export default function RentsView() {
    ...
    const fetchData = async () => {
        try {
            ...
            const formattedData = alquileres.map(alquiler => ({
                title: alquiler.nombreCliente + ' - ' + alquiler.nombreVehiculo,
                subtitle: `Cliente: ${alquiler.nombreCliente}, Vehículo: ${alquiler.nombreVehiculo} - Importe: ${alquiler.precio}`,
                ...alquiler
            }));
                ...
                <Modal animationType="fade" transparent={true} visible={modalVisible}>
                    <View style={styles.overlay}>
                        <View style={styles.modalcontainer}>
                            ...
                            <View style={{ flex: 1 }} />
                            <Text style={styles.modalText}>Cliente: {selectedItem.nombreCliente}</Text>
                            <Text style={styles.modalText}>Vehículo: {selectedItem.nombreVehiculo}</Text>
                            <Text style={styles.modalText}>Importe: {selectedItem.precio} €</Text>
                            <Text style={styles.modalText}>Fecha Inicio: {new Date(selectedItem.FechaInicio).toLocaleDateString()}</Text>
                            <Text style={styles.modalText}>Fecha Fin: {new Date(selectedItem.FechaFin).toLocaleDateString()}</Text>
                            <View style={{ flex: 1 }} />
